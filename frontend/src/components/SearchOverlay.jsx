...
export default function SearchOverlay({ isOpen, onClose, onSelectItem }) {
  ...
  return (
    ...
    <div 
      ...
      style={{ backgroundColor: 'rgba(14, 21, 20, 0.98)' }}
      onClick={onClose}
    >
      {/* Search Header - Fixed at top */}
      <div className="flex-shrink-0 px-3 md:px-4 pt-4 md:pt-6 pb-3 md:pb-4" onClick={(e) => e.stopPropagation()}>
        <div className="max-w-4xl mx-auto">
          <div className="mb-3 md:mb-4 text-center">
            <h2 className="text-base md:text-2xl font-bold text-white mb-1.5 md:mb-2">
          ...
          <div className="flex flex-wrap justify-center gap-1.5 md:gap-2 mb-2 md:mb-3">
            {moodChips.map((chip, idx) => (
              <button
                ...
                className="px-2.5 md:px-4 py-1 md:py-1.5 rounded-full text-[10px] md:text-sm font-medium transition active:scale-95 hover:scale-105"
                style={{ 
          ...
          <div className="relative">
            <input
              ...
              className="w-full px-3 md:px-6 py-2.5 md:py-4 pr-16 md:pr-28 rounded-xl text-sm md:text-lg text-white placeholder-white/50 focus:outline-none focus:ring-2"
              style={{ 
            ...
            <button
              ...
              className="absolute right-1.5 md:right-3 top-1/2 -translate-y-1/2 px-3 md:px-6 py-1.5 md:py-2 rounded-lg text-xs md:text-sm font-medium transition active:scale-95 hover:scale-105"
          ...
          <button
            onClick={onClose}
            className="absolute top-3 md:top-4 right-3 md:right-4 text-white/70 hover:text-white text-xl md:text-2xl z-10"
          >
            ✕
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 md:px-4 pb-20" onClick={(e) => e.stopPropagation()}>
        <div className="max-w-4xl mx-auto">
          {/* Bro's Response */}
          {broResponse && (
            <div 
              className="mb-4 md:mb-6 p-3 md:p-4 rounded-xl text-center italic text-sm md:text-base"
          ...
          {/* Results Grid - NO max-height, fills available space */}
          {results.length > 0 && (
          ...
          {results.length === 0 && broResponse && !loading && (
            <div className="text-center text-white/60 text-xs md:text-sm mt-4">
