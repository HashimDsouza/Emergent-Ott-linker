...
export default function Tile({ item, onInfo }) {
  ...
  return (
    <div onClick={handleClick} className="relative block rounded-2xl overflow-hidden shadow-lg border border-white/10 hover:-translate-y-0.5 transition cursor-pointer">
      ...
      <div className="absolute inset-x-0 bottom-0 p-3 pb-5 text-white">
        {/* 1: Platform + social */}
        <div className="flex items-center gap-3 mb-1 mt-1">
          <span className={badgeCls}>{item.platform || "JioHotstar"}</span>
          <span className="inline-flex items-center gap-1 text-xs opacity-90"><span>❤️</span><span className={counter}>1.2k</span></span>
          <span className="inline-flex items-center gap-1 text-xs opacity-90"><span>👎</span><span className={counter}>120</span></span>
          <span className="inline-flex items-center gap-1 text-xs opacity-90"><span>💬</span><span className={counter}>320</span></span>
        </div>

        {/* 2: Buzz Meter */}
        <div className="flex items-center gap-2 text-[11px] uppercase tracking-wider opacity-85 mb-1">
          <span>Buzz Meter</span>
          <a 
            href={item.buzz?.yt || `https://www.youtube.com/results?search_query=${encodeURIComponent(item.title + ' trailer')}`}
            target="_blank"
          ...
          <a 
            href={item.buzz?.x || `https://twitter.com/search?q=${encodeURIComponent(item.title)}`}
            target="_blank"
            rel="noopener noreferrer"
            onClick={(e) => e.stopPropagation()}
            title="Buzzing on X"
          ...
          <a 
            href={item.buzz?.reddit || `https://www.reddit.com/search/?q=${encodeURIComponent(item.title)}`}
            target="_blank"
            rel="noopener noreferrer"
            onClick={(e) => e.stopPropagation()}
